...
public class DBUtils {
     ...
     * @param sql        执行的语SQL句
     * @param whereMap   where条件
     * @param startWith2 是否从第直接用AND连接条件
     * @return List<Map < String, Object>>
     * @throws SQLException
     */
    public static List<Map<String, Object>> queryLikeMultLimit(String sql,
                                                               Map<String, Object> whereMap, Boolean startWith2) throws Exception {
        StringBuffer SQL = new StringBuffer();
        SQL.append(sql);
        if (whereMap != null && whereMap.size() > 0) {
            Iterator<String> iterator = whereMap.keySet().iterator();
            //i=1就 会从第一个WherMap 开始 添加条件
            int i = startWith2 ? 1 : 0;
            while (iterator.hasNext()) {
                String key = iterator.next();
     ...
     * @throws SQLException
     */
    public static List<Map<String, Object>> queryLikeLikeMultLimit(String sql,
                                                                   Map<String, Object> whereMap, Boolean startWith2) throws Exception {
        StringBuffer SQL = new StringBuffer();
        SQL.append(sql);
        if (whereMap != null && whereMap.size() > 0) {
    ...
    private static List<Map<String, Object>> getDatas(ResultSet resultSet) throws SQLException {
        ...
        for (int i = 0; i < datas.size(); i++) {
            ...
            //todo 显示查询结果System.out.println("第" + (i + 1) + "行：" + map);
        }
        return datas;
    }
